...
class HidDevice(HidDeviceBaseClass):
    ...
    def get_input_report(self):
        raw_report = self._input_report_queue.get()
        if not raw_report:
            return None
        self._input_report_queue.reuse(raw_report)
        report = []
        for i in range(self.hid_caps.input_report_byte_length):
            report.append(raw_report[i])
        return report
                ...
                while not self.__abort:
                    ...
                    if result == winapi.ERROR_IO_PENDING:
                        ...
                        if result != winapi.WAIT_OBJECT_0 or self.__abort: #success
                            ...
                            #pass
                        result = winapi.GetOverlappedResult(hid_object.hid_handle,byref(over_read),byref(bytes_read),True)
